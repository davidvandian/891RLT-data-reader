...
int main() {
    ...
    if ((Zfiledir.parent_path() == θfiledir.parent_path()) && ynq("Save result in same directory?")) {
        dir = Zfiledir.parent_path();
    }
    else {
        fileinput("result", dir);
        while (std::filesystem::status(dir).type() != std::filesystem::file_type::directory) {
            std::cout << "\nNot a folder! ";
            fileinput("again", dir);
        }
    }

    std::ofstream output{ dir.concat(newfilename) };
